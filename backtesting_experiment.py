...
def run_experiment():
    ...
    length_of_experiment = 15  # Number of years in each evaluation period
    earliest_year_in_data = 1980  # We don't really trust pre 1980 data from statmuse
    portfolio_compositions = [1, 5, 10, 15, 20, 25]  # Cannot get more than 25 from statmuse

    ## Setup
    portfolio_results = {n: [] for n in portfolio_compositions}
    portfolio_percent_return_results = {n: [] for n in portfolio_compositions}
    ...
    for starty, endy in periods:
        ...
        for n in portfolio_compositions:
            ...
            portfolio_percent_return_results[n].append(avg_annual_return_n)
    ...
    print_experiment_results(results, portfolio_compositions, portfolio_results, best_portfolio_counts, baseline_results, baseline_apr, portfolio_percent_return_results)

def print_experiment_results(results, portfolio_compositions, portfolio_results, best_portfolio_counts, baseline_results, baseline_apr, portfolio_percent_return_results):
    print("\n\n\n")
    ...
    for result in results:
        ...
        best_portfolio = max(period_results, key=lambda x: x[1])
        print(f"Best performing portfolio: {best_portfolio[2]} with return: %{100*best_portfolio[0]:.2f}")
        print("\n")
    ...
    for n in portfolio_compositions:
        ...
        avg_apr = sum(portfolio_percent_return_results[n]) / len(portfolio_percent_return_results[n])
        print(f"Top {n} stocks average APR: %{100 * avg_apr:.2f}")
